...
int main(int argc, char *argv[]) {
    ...
    errno_t err;
    err = fopen_s(&fp, "D:/lab_105/ReportS1a.txt", "a+");
...
if (argv[1]) {
 ...
 // printf( " input n = %d " , n);
  int samples=6; //5 muestras y se ignora la primera
    ...
    printf("version  #sample    n    time(s)        Normalized(ns)\n");
  for(int s=0;s<samples;s++){
    // Start measuring time
            clock_t start = clock();
            multMat3(n, A, B, C);     //select multMatv1, multMatv2, multMatv3, multMatv4
    ...
    printf( "%s\t  %03d\t %05d\t  %2.4f\t   %2.4f \n" , "ver3f ", s,n, seconds, timeNormalized);
    fprintf_s(fp, "%s\t %03d\t %05d  %2.4f\t   %2.4f \n" , "ver3f ",s, n, seconds, timeNormalized);    /* write data to the file */
  }
  //PrintMat(n, A);
  //PrintMat(n, B);
  //PrintMat(n, C);
